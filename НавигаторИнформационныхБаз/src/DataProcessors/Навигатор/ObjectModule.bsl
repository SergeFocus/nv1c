...
Процедура ДобавитьПользователейБазы(СтруктураМД)
	...
	стр_Коллекция = СтруктураМД.Строки.Добавить();
	
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмяКоллекции = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции = ИмяЭлементаКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмя = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииПредставление = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииСиноним = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииНомерКартинки = НомерКартинки;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииТипЗаписи = "КоллекцияОбъектовМетаданных";
	...
	Для каждого ПользовательБазы Из ВсеПользователи Цикл
		...
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмяКоллекции = ИмяКоллекции;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции = ИмяЭлементаКоллекции;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмя = ПользовательБазы.Имя;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииПредставление = ПользовательБазы.Имя;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииСиноним = ПользовательБазы.ПолноеИмя;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииТипЗаписи = "ПользовательИнформационнойБазы";
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииНомерКартинки = НомерКартинки;
	КонецЦикла;
	
	стр_Коллекция.Строки.Сортировать("СтруктураМетаданныхКонфигурацииИмя");
	
КонецПроцедуры

Процедура ДобавитьКоллекциюОбъектовМетаданных(СтруктураМД, СвойстваКоллекции)
	
	ПолноеИмяКоллекции = СвойстваКоллекции.ИмяКоллекции;
	...
	Если ЧастиИмяКоллекции.Количество() > 1 Тогда
		...
		строки_СтруктураМД = СтруктураМД.Строки.НайтиСтроки(
			Новый Структура("СтруктураМетаданныхКонфигурацииТипЗаписи, СтруктураМетаданныхКонфигурацииИмя", 
			"ГруппаКоллекцийОбъектовМетаданных", ИмяГруппы)
		);
		
		Если строки_СтруктураМД.Количество() = 0 Тогда
			
			стр_Группа = СтруктураМД.Строки.Добавить();
			стр_Группа.СтруктураМетаданныхКонфигурацииТипЗаписи = "ГруппаКоллекцийОбъектовМетаданных";
			стр_Группа.СтруктураМетаданныхКонфигурацииИмяКоллекции = ИмяГруппы;
			стр_Группа.СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции = ИмяГруппы;
			стр_Группа.СтруктураМетаданныхКонфигурацииИмя = ИмяГруппы;
			стр_Группа.СтруктураМетаданныхКонфигурацииПредставление = ИмяГруппы;
			стр_Группа.СтруктураМетаданныхКонфигурацииСиноним = ИмяГруппы;
			стр_Группа.СтруктураМетаданныхКонфигурацииНомерКартинки = 2;
			
		Иначе
			стр_Группа = строки_СтруктураМД[0];
		КонецЕсли; 
	...
	стр_Коллекция.СтруктураМетаданныхКонфигурацииТипЗаписи = "КоллекцияОбъектовМетаданных";
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмяКоллекции = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции = СвойстваКоллекции.ИмяЭлементаКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииИмя = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииПредставление = ИмяКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииСиноним = СвойстваКоллекции.СинонимКоллекции;
	стр_Коллекция.СтруктураМетаданныхКонфигурацииНомерКартинки = СвойстваКоллекции.НомерКартинки;
	
	Для каждого ОбъектМд Из Метаданные[ИмяКоллекции] Цикл
		стр_ОбъектМетаданных = стр_Коллекция.Строки.Добавить();
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмяКоллекции = ИмяКоллекции;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции = СвойстваКоллекции.ИмяЭлементаКоллекции;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииИмя = ОбъектМд.Имя;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииПредставление = ОбъектМд.Имя;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииСиноним = ОбъектМд.Синоним;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииКомментарий = ОбъектМд.Комментарий;
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииТипЗаписи = "ОбъектМетаданных";
		стр_ОбъектМетаданных.СтруктураМетаданныхКонфигурацииНомерКартинки = СвойстваКоллекции.НомерКартинки;
	КонецЦикла;
	
КонецПроцедуры

Процедура ДобавитьЗаписьОписанияКоллекцииОМ(ОписаниеКоллекции, ИмяКоллекции, ИмяЭлементаКоллекции, СинонимКоллекции, НомерКартинки = 0)
...
Функция СтруктураМетаданныхКонфигурации() Экспорт
	...
	СтруктураМД = Тип_СтруктураМетаданныхКонфигурации();
	СтруктураМД.Строки.Очистить();
	
	ВсеКоллекции = КоллекцииОбъектовМетаданных();
	
	Для каждого Коллекция Из ВсеКоллекции Цикл
		ДобавитьКоллекциюОбъектовМетаданных(СтруктураМД, Коллекция);
	КонецЦикла;
	
	ДобавитьПользователейБазы(СтруктураМД);
	
	Результат = СтруктураМД;
...
Функция КоллекцииОбъектовМетаданных()
	...
	// Общие
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.Подсистемы",             "Подсистема",      "Подсистемы", 20);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеМодули",             "ОбщийМодуль",      "Общие модули", 21);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ПараметрыСеанса",         "ПараметрСеанса",   "Параметры сеанса", 22);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.Роли",                    "Роль",             "Роли", 23);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеРеквизиты",          "ОбщийРеквизит",    "Общие реквизиты", 24);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ПланыОбмена",             "ПланОбмена",       "Планы обмена", 25);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.КритерииОтбора",          "КритерийОтбора",    "Критерии отбора", 26);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ПодпискиНаСобытия",       "ПодпискаНаСобытие", "Подписки на события", 27);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.РегламентныеЗадания",     "РегламентноеЗадание", "Регламентные задания", 28);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ФункциональныеОпции",     "ФункциональнаяОпция", "Функциональные опции", 29);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ПараметрыФункциональныхОпций",     "ПараметрФункциональнойОпции", "Параметры функциональных опций", 30);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОпределяемыеТипы",     "ОпределяемыйТип", "Определяемые типы", 31);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ХранилищаНастроек",     "ХранилищеНастроек", "Хранилища настроек", 32);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеФормы",     "ОбщаяФорма", "Общие формы", 33);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеКоманды",     "ОбщаяКоманда", "Общие команды", 34);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ГруппыКоманд",     "ГруппаКоманд", "Группы команд", 35);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеМакеты",     "ОбщийМакет", "Общие макеты", 36);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ОбщиеКартинки",     "ОбщаяКартинка", "Общие картинки", 37);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ПакетыXDTO",     "ПакетXDTO", "XDTO-пакеты", 38);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.WebСервисы",     "WebСервис", "Web-сервисы", 39);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.HTTPСервисы",     "HTTPСервис", "HTTP-сервисы", 40);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.WSСсылки",     "WSСсылка", "WS-ссылки", 41);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.ЭлементыСтиля",     "ЭлементСтиля", "Элементы стиля", 42);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Общие.Языки",     "Язык", "Языки", 43);
	
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Константы", "Константа", "Константы", 3);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Справочники", "Справочник", "Справочники", 4);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Документы", "Документ", "Документы", 5);
	...
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "ЖурналыДокументов",       "ЖурналДокументов", "Журналы документов", 6);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Перечисления",            "Перечисление",     "Перечисления", 7);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Отчеты",                  "Отчет", "Отчеты", 8);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Обработки",               "Обработка", "Обработки", 9);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "ПланыВидовХарактеристик", "ПланВидовХарактеристик", "Планы видов характеристик", 10);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "ПланыСчетов",             "ПланСчетов", "Планы счетов", 11);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "ПланыВидовРасчета",       "ПланВидовРасчета", "Планы видов расчета", 12);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "РегистрыСведений",        "РегистрСведений", "Регистры сведений", 13);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "РегистрыНакопления",      "РегистрНакопления", "Регистры накопления", 14);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "РегистрыБухгалтерии",     "РегистрБухгалтерии", "Регистры бухгалтерии", 15);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "РегистрыРасчета",         "РегистрРасчета", "Регистры расчета", 16);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "БизнесПроцессы",          "БизнесПроцесс", "Бизнес-процессы", 17);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "Задачи",                  "Задача", "Задачи", 18);
	ДобавитьЗаписьОписанияКоллекцииОМ(Результат, "ВнешниеИсточникиДанных",  "ВнешнийИсточникДанных", "Внешние источники данных", 19);
...
//
// Параметры:
//	ИмяКоллекции - Строка - имя коллекции объектов метаданных, например: "Справочники", "Документы".
//
// Возвращаемое значение:
//	Тип: Булево
//	Для коллекции доступно отображение списка элементов.
//
...
// Возвращаемое значение:
//	Тип: Строка
//	Имя основной таблицы списка элементов объекта метаданных, например: "Справочник.Номенклатура".
//	
...
Функция ПараметрыКолонокСпискаЭлементовОМ(ИмяКоллекции, ИмяОбъектаМетаданных) Экспорт
	...
	Если ИмяКоллекции = "ПланыОбмена" Тогда
		...
		ПараметрыКолонки = Тип_ПараметрыКолонкиСпискаЭлементовОМ(СтРеквизит.Имя, СтРеквизит.Синоним, СтРеквизит.Комментарий, СтРеквизит.Тип, Истина);
		...
		Для Каждого СтРеквизит Из ОбъектМд.СтандартныеРеквизиты Цикл
			...
			ПараметрыКолонки = Тип_ПараметрыКолонкиСпискаЭлементовОМ(СтРеквизит.Имя, СтРеквизит.Синоним, СтРеквизит.Комментарий, СтРеквизит.Тип, ВидимостьРеквизита);
			СтандартныеРеквизиты.Добавить(ПараметрыКолонки);
		...
		Для Каждого Реквизит Из ОбъектМд.Реквизиты Цикл
			...
			ПараметрыКолонки = Тип_ПараметрыКолонкиСпискаЭлементовОМ(Реквизит.Имя, Реквизит.Синоним, Реквизит.Комментарий, СтРеквизит.Тип, Ложь);
	...
	ИначеЕсли ИмяКоллекции = "Справочники" Тогда
		...
		Если ОбъектМд.ДлинаНаименования > 0 Тогда
			...
			ПараметрыКолонки = Тип_ПараметрыКолонкиСпискаЭлементовОМ(СтРеквизит.Имя, СтРеквизит.Синоним, СтРеквизит.Комментарий, СтРеквизит.Тип, Ложь);
			СтандартныеРеквизиты.Добавить(ПараметрыКолонки);
		...
		Для Каждого Реквизит Из ОбъектМд.Реквизиты Цикл
			...
			ПараметрыКолонки = Тип_ПараметрыКолонкиСпискаЭлементовОМ(Реквизит.Имя, Реквизит.Синоним, Реквизит.Комментарий, Реквизит.Тип, Ложь);
			Реквизиты.Добавить(ПараметрыКолонки);
...
Функция ИменаТабличныхЧастейОбъектаМетаданных(ОбъектМд) Экспорт
	...
	Для Каждого ТабличнаяЧастьОбъекта Из ОбъектМд.ТабличныеЧасти Цикл
...
//
// Параметры:
//	ОбъектМд - ОбъектМд - метаданные объекта, например справочника, документа,
//	ИмяТабличнойЧасти - Строка - имя таблийно части объекта метаданных, например: "Товары".
//
// Возвращаемое значение:
//	Тип: Строка
//	Полное имя табличной части объекта метаданных, например: "Документ.ПриходнаяНакладная.Товары".
// 			 
Функция ПолноеИмяТабличнойЧастиОбъектаМетаданных(ОбъектМд, ИмяТабличнойЧасти) Экспорт
	...
	ИмяТипаОбъекта = ИмяТипаОбъектаМетаданных(ОбъектМд);
	
	Если (Не ЗначениеЗаполнено(ИмяТипаОбъекта)) Тогда
		Возврат ПустойРезультат;
	КонецЕсли;
	
	Результат = ПодставитьПараметрыВСтроку("%1.%2.%3", ИмяТипаОбъекта, ОбъектМд.Имя, ИмяТабличнойЧасти);
	
	Возврат Результат;
	
КонецФункции

Функция ИмяТипаОбъектаМетаданных(ОбъектМд)
...
Функция КоличествоЭлементовКоллекцииТипаОбъектаМетаданных(ИмяКоллекцииМетаданных, ИмяОбъектаМетаданных) Экспорт
	...
	Запрос = КоличествоЭлементовКоллекцииТипаОбъектаМетаданных_Запрос(ИмяКоллекцииМетаданных, ИмяОбъектаМетаданных);
...
Функция КоличествоЭлементовКоллекцииТипаОбъектаМетаданных_Запрос(ИмяКоллекцииМетаданных, ИмяОбъектаМетаданных)
	
	Результат = Новый Запрос;
	ПустойРезультат = Неопределено;
	
	ТекстЗапроса = КоличествоЭлементовКоллекцииТипаОбъектаМетаданных_ТекстЗапроса(ИмяКоллекцииМетаданных, ИмяОбъектаМетаданных);
...
Функция КоличествоЭлементовКоллекцииТипаОбъектаМетаданных_ТекстЗапроса(ИмяКоллекцииМетаданных, ИмяОбъектаМетаданных)
	...
	Иначе
		...
		Для каждого ТипЗначения Из Типы Цикл
			...
			ИначеЕсли ТипЗначения = ТипЗнч("") Тогда
				СтрокаОписания = СтрокаОписания + ПодставитьПараметрыВСтроку("Строка (%1,%2)", Тип.КвалификаторыСтроки.Длина, ?(Тип.КвалификаторыСтроки.ДопустимаяДлина = ДопустимаяДлина.Переменная, "*", "")) + ";";
			ИначеЕсли ТипЗначения = ТипЗнч(0) Тогда
				СтрокаОписания = СтрокаОписания + ПодставитьПараметрыВСтроку("Число (%1,%2,%3)", Тип.КвалификаторыЧисла.Разрядность, Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти, ?(Тип.КвалификаторыЧисла.ДопустимыйЗнак = ДопустимыйЗнак.Любой, "+-", "+")) + ";";
			ИначеЕсли ТипЗначения = ТипЗнч(Истина) Тогда
				СтрокаОписания = СтрокаОписания + "Булево" + ";";
			ИначеЕсли ТипЗначения = ТипЗнч('00010101') Тогда
				СтрокаОписания = СтрокаОписания + ПодставитьПараметрыВСтроку("Дата (%1)", ?(Тип.КвалификаторыДаты.ЧастиДаты = ЧастиДаты.Дата, "Д", ?(Тип.КвалификаторыДаты.ЧастиДаты = ЧастиДаты.Время, "В", "Д+В"))) + ";";
			Иначе
...
Функция СтрокаИзМассиваПодстрок(Знач ЧастиСтроки, Знач Разделитель = "", ПропускатьПустыеСтроки = Ложь, СокращатьНепечатаемыеСимволы = Ложь) Экспорт
...
Функция Тип_СтруктураМетаданныхКонфигурации() Экспорт
	...
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииТипЗаписи"); // Тип записи структуры метаданных конфигурации, например: "КоллекцияОбъектовМетаданных", "ОбъектМетаданных"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииИмяКоллекции"); // Имя коллекции, например: "Справочники"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииИмяЭлементаКоллекции"); // Имя элемента коллекции, например: "Справочник"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииИмя"); // Имя объекта метаданных, например: "Номенклатура"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииПредставление");
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииСиноним"); // Синоним имени объекта метаданных, например: "Номенклатура"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииКомментарий"); // Имя объекта метаданных, например: "Номенклатура"
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииКоличествоЗаписей"); // Количество записей элементов данного вида, например количество записей в справочнике
	Результат.Колонки.Добавить("СтруктураМетаданныхКонфигурацииНомерКартинки");
...
Функция Тип_ПараметрыКолонкиСпискаЭлементовОМ(ИмяРеквизита = "", Синоним = "", Комментарий = "", ТипЗначенияКолонки = Неопределено, Видимость = Ложь) Экспорт
...
РазделительЧастейИмен = "_777_";

#Область НомераКартинок

// Номера картинок общей коллекции:
// 0 - пользователь
// 1 - конфигурация
// 2 - общие
// 3 - константа
...
// 8 - отчет
// 9 - обработка
// 10 - план видов характеристик
// 11 - план счетов
// 12 - план видоа расчета
// 13 - регистр сведений
// 14 - регистр накопления
// 15 - регистр бухгалтерии
// 16 - регистр расчета
// 17 - бизнес-процесс
// 18 - задача
// 19 - внешний источник данных
// 20 - подсистема
// 21 - общие модули
// 22 - параметры сеанса
// 23 - роль
...
// 25 - планы обмена
// 26 - критерии отбора
// 27 - подписки на события
// 28 - регламентные задания
// 29 - функциональные опции
// 30 - параметры функциональных опций
// 31 - определяемые типы
// 32 - хранилища настроек
// 33 - общие формы
// 34 - общие команды
// 35 - группы команд
// 36 - общие макеты
...
// 42 - элементы стиля
// 43 - языки
// 44 - стандартные реквизиты
// 45 - значения перечисления, реквизиты
// 46 - табличные части
// 47 - измерения
// 48 - ресурсы
// 49 - пользователь
