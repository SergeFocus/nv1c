...
Процедура ДобавитьКоллекциюОбъектовМетаданных(СтруктураМетаданных, СвойстваКоллекции)
	...
	Если ИмяКоллекции = "Подсистемы" Тогда
		...
		Для каждого Подсистема Из Метаданные[ИмяКоллекции] Цикл
			ДобавитьСведенияОПодсистеме(стр_Коллекция, СвойстваКоллекции, Подсистема);
		КонецЦикла;
...
Процедура ДобавитьСведенияОПодсистеме(стр_Коллекция, СвойстваКоллекции, Подсистема)
	...
	Если Подсистема.Подсистемы.Количество() > 0 Тогда
		Для каждого ВложеннаяПодсистема Из Подсистема.Подсистемы Цикл
			ДобавитьСведенияОПодсистеме(стр_Подсистема, СвойстваКоллекции, ВложеннаяПодсистема);
		КонецЦикла;
	КонецЕсли;
...
//	ЛюбаяСсылка. Ссылка не элемент информационной базы.
//
Функция ПолучитьСсылкуНаЭлементПоИдентификатору(СтрокаGUID,
	ОпределительТипаЭлементов) Экспорт
	...
	Результат = МенеджерОбъектовИБ.ПолучитьСсылку(УникальныйИд);
...
Функция НавигационнаяСсылкаНаУзелПланаОбмена(Узел) Экспорт
	...
	Если Ложь Тогда
		Узел = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.СоздатьУзел();
	КонецЕсли;
