...
Процедура ЗаполнитьВсеДвижения()
	...
	// TODO: Требуется рефакторинг. Переделать на универсальное заполнение.
	// Работа с реквизитами всех регистров дожна быть единой.
	...
	дз_ВсеРегистрыБухгалтерии.Очистить();
	...
	ЭтоДокумент = Метаданные.Документы.Содержит(ОбъектМд);
	...
	Если ЭтоДокумент Тогда
		...
		Для каждого РегистрМд Из ОбъектМд.Движения Цикл
			...
			Если Метаданные.РегистрыСведений.Содержит(РегистрМд) Тогда
			...
			Если Метаданные.РегистрыБухгалтерии.Содержит(РегистрМд) Тогда

				стр_Регистр = дз_ВсеРегистрыБухгалтерии.Добавить();
				стр_Регистр.РегистрБухгалтерииИмя = РегистрМд.Имя;
				стр_Регистр.РегистрБухгалтерииСиноним = РегистрМд.Синоним;
				//стр_Регистр.РегистрБухгалтерииТипСтроки = "РегистрБухгалтерии";
				//стр_Регистр.РегистрБухгалтерииНомерКартинки = 14;
			...
			Если Метаданные.РегистрыРасчета.Содержит(РегистрМд) Тогда

				стр_Регистр = дз_ВсеРегистрыРасчета.Добавить();
				стр_Регистр.РегистрРасчетаИмя = РегистрМд.Имя;
				стр_Регистр.РегистрРасчетаСиноним = РегистрМд.Синоним;
				//стр_Регистр.РегистрРасчетаТипСтроки = "РегистрБухгалтерии";
	...
	дз_ВсеРегистрыБухгалтерии.Сортировать("РегистрБухгалтерииИмя");
...
Функция ИмяРеквизитаФормыТаблицаОбъекта(ТипТаблицы, ИмяТаблицы)
	...
	Результат = ЭтаОбработка().ПодставитьПараметрыВСтроку("%1" + Объект.Система_РазделительЧастейИмен + "%2", ТипТаблицы, ИмяТаблицы);
...
Функция ТекстЗапросаСпискаТаблицыОбъектаКонфигурации(ТипТаблицы, ИмяТаблицы)
	...
	Для Каждого СтРеквизит Из КоллекцияМд.СтандартныеРеквизиты Цикл
		...
		Если ТипТаблицы = "РегистрБухгалтерии" Тогда
			...
			СинонимПоля = ТипТаблицы + Объект.Система_РазделительЧастейИмен + ИмяТаблицы + Объект.Система_РазделительЧастейИмен + СтРеквизит.Имя;
	...
	Если ТипТаблицы = "РегистрНакопления" Или ТипТаблицы = "РегистрСведений" Или ТипТаблицы = "РегистрБухгалтерии" Тогда
		...
		Для Каждого Реквизит Из КоллекцияМд.Измерения Цикл
			...
			СинонимПоля = ТипТаблицы + Объект.Система_РазделительЧастейИмен + ИмяТаблицы + Объект.Система_РазделительЧастейИмен + Реквизит.Имя;
...
Функция ИмяРеквизитаФормыКолонкаТаблицыОбъекта(ТипТаблицы, ИмяТаблицы, ИмяРеквизитаТаблицы)
	...
	Результат = ТипТаблицы + Объект.Система_РазделительЧастейИмен + ИмяТаблицы + Объект.Система_РазделительЧастейИмен + ИмяРеквизитаТаблицы;
...
Функция ИмяСтраницыТаблицыОбъекта(ТипТаблицы, ИмяТаблицы)
	...
	ЭтаОбработка = ЭтаОбработка();
	Результат = ЭтаОбработка.ПодставитьПараметрыВСтроку("Страница" + Объект.Система_РазделительЧастейИмен + "%1" + Объект.Система_РазделительЧастейИмен + "%2", ТипТаблицы, ИмяТаблицы); 
